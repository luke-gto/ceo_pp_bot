...
class whitelist_user(telebot.custom_filters.SimpleCustomFilter):
    ...
    def check(message: telebot.types.Message):
        if (
            str(bot.get_chat_member(message.chat.id, message.from_user.id).user.id)
            in whitelisted_ids
        ):
            return True
...
def send_welcome(message):
    bot.reply_to(
        message,
        "Non ammazzate il mio padrone @luke_go se non funziono come dovrei pls.\n<b>Al momento solo LVI può inviarmi i comandi.</b>\n"
        + "Il database verrà salvato nella cartella: %s" % (script_directory + "/data"),
        parse_mode="HTML",
    )
...
def new_session(message):
    message = bot.reply_to(
        message,
        "Inserisci la data di oggi (DD-MM-YYYY). Potrei farlo manualmente ma se la partita dura oltre la mezzanotte... implodo!",
    )
...
def send_polls(input, message):
    q1 = bot.send_poll(
        message.chat.id, input + cat1, options, open_period=60, is_anonymous=True
    )
    q2 = bot.send_poll(
        message.chat.id, input + cat2, options, open_period=60, is_anonymous=True
    )
    q3 = bot.send_poll(
        message.chat.id, input + cat3, options, open_period=60, is_anonymous=True
    )
...
def send_save(message):
    bot.reply_to(
        message,
        "N° voti castati: "
        + str(get_poll_data(date_session, message.reply_to_message)),
...
def send_results(message):
    ...
    bot.reply_to(
        ...
        % (generale, ridere, sapere, sacrificio),
        parse_mode="MARKDOWN",
    )
    ...
    bot.send_document(
        message.chat.id,
        db,
        caption="Questo è il database con i dati. Lo puoi aprire con un qualunque software che supporti SQLite. Io consiglio [DB Browser for SQLite](https://sqlitebrowser.org/).",
        parse_mode="MARKDOWN",
    )
